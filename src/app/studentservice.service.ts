...
export class StudentserviceService {
  ...
  liststudents():Observable<Student[]>{
    return this.http.get<Student[]>("http://localhost:8585/students");
  }
  ...
  update(stu : Student) : Observable<Student>{
    return this.http.put<Student>("http://localhost:8585/update",stu,myheader);
  }

  getStudentById(id:number) : Observable<Student>{
    return this.http.get<Student>("http://localhost:8585/studentById/"+id);
  }
}
